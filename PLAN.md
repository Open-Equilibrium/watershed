...
- Optional clients for one or more Meta-Harness instances. Liquid presents a unified projection but preserves instance identity, freshness and authority; live commands route to the owning instance and fail clearly when it is unreachable. Liquid does not implement a session backend, config resolver, scheduler, AgentPulse engine, adapter layer or direct CLI-agent process manager (boundaries: D-025, D-027).
