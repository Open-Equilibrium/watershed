...
- **ADR-0067** | 2026-07-17 | Accepted | Keep the 10 MiB active-loop memory budget: scan a bounded registry catalog, retain each definition in the selected Loop's transitive closure once, and enforce smaller definition/closure limits with a near-limit RSS gate; this supersedes ADR-0054's per-file registry cap — Loop Agent V-Spec, `PERFORMANCE.md`.
