...
- **ADR-0050** | 2026-06-24 | Accepted | M1 context handling is deterministic and excludes embeddings, RAG and adaptive compaction; ADR-0058 fixes the exact tiered compiler/profile — Loop Agent V-Spec.
...
- **ADR-0058** | 2026-07-13 | Accepted | Loop Agent M1 uses the single deterministic, cache-stable `loop-context-v0` profile: mandatory active-scope context, narrowly bounded continuity, reproducible manifests/hashes and no provider-side truncation; persisted compaction and retrieval remain post-M1 — Loop Agent V-Spec.
