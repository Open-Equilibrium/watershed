...
- **ADR-0081** | 2026-07-25 | Accepted; supersedes ADR-0076/ADR-0077 physical session-lock ownership and crash-recovery clauses | D-049 option C: M1 session ownership is an exclusive OS-held lease in a host-local coordinator keyed by canonical workspace and session ID; direct workspace writers are in scope, `.flow/sessions/<session_id>.lock` is a persistent observable marker only, marker mutation cannot grant or revoke ownership, and process exit releases the lease without marker cleanup — `PROTOCOL.md`, `SECURITY.md`, Flow Agent V-Spec.
- **ADR-0082** | 2026-07-25 | Accepted | D-050 option A: the M1 host-local coordinator is a private deterministic sibling of the canonical workspace, independent of process temporary-directory settings; the canonical workspace parent is its trust and access boundary, and unavailable coordinator access fails before workspace side effects — `PROTOCOL.md`, `SECURITY.md`.
