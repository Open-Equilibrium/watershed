...
**Status: replacement integrated.** The [security contract](../../SECURITY.md#accepted-flow-agent-security-target) is normative. Linux Bubblewrap/seccomp and Mac Seatbelt implement the narrow replacement, including selected-home protection and warned overlapping locations. The diagrams explain the required boundary; they are not runtime verification. [TESTING.md](../../TESTING.md#native-verification-status) owns native results and acceptance gaps. Future features belong only to the [roadmap](../../PLAN.md#later-flow-agent-roadmap).
...
Runtime, policy/schema, wire and fixtures have been migrated. The [native test matrix and results](../../TESTING.md#native-verification-status) cover Ubuntu 24.04 x86_64 and macOS 26 ARM64 instead of the prior systemd/container gate. Historical feasibility and passing Windows shared tests cannot establish complete protected-set admission, publication races, child lifetime or native installation correctness. Download-distribution and final review remain separate acceptance gates.
