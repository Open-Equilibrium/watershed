# Native Flow-file protection

**Replacement integrated.** [SECURITY.md](../../SECURITY.md#accepted-flow-agent-security-target) owns the guarantees and exclusions. This design owns this installation's protected inventory and admission invariant for Linux Bubblewrap/seccomp and Mac Seatbelt. [PROTOCOL.md](../../PROTOCOL.md#m12-executor-protocol-adr-0146-adr-0160-adr-0161-adr-0162) owns the integrated wire/backend mechanics; [TESTING.md](../../TESTING.md#native-verification-status) owns native results and acceptance gaps. Future features belong only to the [roadmap](../../PLAN.md#later-flow-agent-roadmap).

## Selected mechanism and user consequences

The macOS backend uses the selected profile mechanism as part of checked launch admission. The short-lived Default Executor requires no Linux VM, permanent service or additional product runtime on Mac. `/usr/bin/sandbox-exec` is supplied by macOS; Python drives development probes only. Linux applies the same protected-set contract through the [native backend mechanics](../../PROTOCOL.md#native-backend-contract). Both require native product proof.
...
Ordinary non-interactive shell commands and project builds remain the intended use case. Tools receive bounded input/output pipes, not the human review terminal. The accepted terminal-device denial can break programs requiring direct terminal access, interactive password prompts or terminal-user-interface libraries. It is not a general promise that every Mac application works. Native GUI automation through independent services and Metal compatibility require their own evidence; neither is established by a C compilation test.

## Native App Sandbox comparison

This section retains the bounded feasibility comparison that informed ADR-0172. [TESTING.md](../../TESTING.md#authorized-mac-feasibility-evaluation-adr-0167) owns reproducible commands, source revisions, counts and hosted ARM64 observations. The experiment involved no production Executor, administrator service or release-signing identity; it does not verify the integrated replacement.
...
The comparison includes the native-helper matrix and three npm workloads per configuration, including the runtime-read variant. Exact revisions and outcome counts belong to [TESTING.md](../../TESTING.md#authorized-mac-feasibility-evaluation-adr-0167). All unprotected mutation/control operations succeeded. No table entry is a general application-certification claim. Both original project-exception variants produced the same functional outcomes below.
...
All three unprotected controls completed and performed their deliberate writes. Completing the observation matrix does not turn the measured App restrictions or raw-profile counterexamples into passing security tests. A general claim that npm cannot work in App Sandbox would be false.
...
| Concern | Codex CLI | Selected Flow-specific profile | Evaluated App Sandbox |
|---|---|---|---|
| Scope | Configurable broader filesystem/network policy plus separate approvals | Narrow mandatory Flow-owned direct-write and accepted terminal restrictions; Tool effects otherwise trusted | Restricted app authority plus explicit resource grants |
...
| Authority expansion | Approval and full-access modes are configurable product policies | Never lift own-file protection; manual configuration administration in the first Flow Agent release, controlled proposals later | No automatic unsandboxed fallback is approved |
| Integration cost | A larger permission engine with platform and tool compatibility rules | Smaller contract, but still requires anchored admission, handle hygiene and native tests; configuration review is deferred | App packaging, entitlements, resource grants and host-workflow adaptations |
...
**Reuse boundary:** the selected design uses the architectural pattern. Flow retains its one-shot Default Executor and narrow policy; native tests must independently prove Flow-specific invariants. A broader Codex-like sandbox would add a different product contract and maintenance work without removing the shared deprecation risk. Copying upstream code would require a separate dependency/licensing review; it is unnecessary for evaluating the mechanism. Neither native approach has a measured general performance advantage here; both avoid a Linux guest, while their different permissions primarily affect compatibility.

## Overlapping homes and editable instructions

ADR-0172 allows deliberate home/installation placement inside a Tool-writable directory with a clear warning. Identify the Flow objects that remain protected and the parent-folder moves/deletion consequently unavailable to Tools. The integrated admission path retains protection and warns about the overlap; the historical nested-path probe demonstrates only the primitive, not complete product admission. An outside hardlink or incomplete inventory remains a start error, not a warning-only exception.
...
Resolve the following from trusted controller/installation configuration before Tool launch, never from model output or a Tool-supplied exclusion. Anchor canonical objects using the existing no-follow filesystem discipline. Executor starts follow the [ADR-0176 launch checks](../../PROTOCOL.md#process-and-framing-contract) and identical Linux/macOS [installation-trust limits](../../SECURITY.md#native-self-protection-and-its-limits).
...
| Installed program objects | Under ADR-0175, always require and protect current `flow` and the selected Executor, including every covered name of the same file. Default selection requires the official sibling `flow-executor`; explicit Custom selection permits intentional omission of that sibling. An existing sibling must also pass admission and be protected, including with Custom selection; an unsafe existing sibling must never be treated as absent. Reject missing, unresolved, unsafe or replaced required programs. Custom code still belongs to the trusted installation, not third-party certification. |
...
The required invariant depends on all of the following; native acceptance must verify their integration:
...
Existing Flow processes must coordinate initial admission with their protected-file publication operations using an installation-owned lease and anchored identities. A scan racing a publisher is retried within a bounded admission attempt or rejected; it is not a valid snapshot. Do not serialize entire Runs or ordinary project work behind that admission lease. Do not rescan all retained history for every Tool call: retain one verified controller admission and preserve it through checked publisher operations. Retain measurements of the actual scanner and contention under [PERFORMANCE.md](../../PERFORMANCE.md); the small fixture scan supplies no large-history performance claim.

The integrated incremental admission and descriptor contract are recorded in [PROTOCOL.md](../../PROTOCOL.md#m12-executor-protocol-adr-0146-adr-0160-adr-0161-adr-0162). The inventory covers the selected home, platform stores and required program images; [TESTING.md](../../TESTING.md#native-verification-status) records native verification.
...
- **Prepare:** initialize required Flow-owned directories through trusted setup, acquire admission, verify identities/alias coverage and build an immutable protected set. Missing prerequisites or a path that cannot be represented safely are explicit readiness failures. The Mac backend uses fixed profile structure and parameter-bound path data; shell interpolation and embedding untrusted path text as profile source are unnecessary.
- **Start:** the existing controller/one-shot Executor seam retains durable intent, `Ready`/`Start` ordering, bounded transport and exact request binding. Install the native restriction and remove all unintended descriptors before the first Tool instruction. Launch trusted enforcement code with a separate sanitized environment; apply Engineer-admitted Tool environment values only inside the established boundary, never to the enforcement launcher's loader or startup hooks. The resolved-policy digest binds the exact protected descriptor/path/identity records and invocation; the validated receipt requires `self_protection_active: true`. Neither attests general containment or complete descendant cleanup.
...
- **Install:** finalize program publication before a new installation starts Tools; active installations follow the [ADR-0176 maintenance restriction](../../SECURITY.md#native-self-protection-and-its-limits). Current installation does not upgrade existing programs. An external hardlink to a selected program is a readiness error; an interrupted install must be finalized or deliberately repaired by its owner. This design does not add an automatic updater or change system/kernel policy.
...
| Product admission | Verify the integrated anchored discovery and coordinated publication through real runtime/installer concurrency and interruption tests. The fixture scanner is deliberately single-owner; its passing tests do **not** prove race-safe production admission. |
| Executor launch | On native Linux x86_64 and macOS ARM64, prove readiness, outer and inner Executor starts, valid Tools, direct-write denial and rejection of detected installation changes at each launch boundary without fallback. Preserve inherited-handle cleanup and pass the native installation, release-artifact, coverage and performance gates in [TESTING.md](../../TESTING.md). |
| Distribution | Build the actual download package; verify checksum failure paths, clean installation and quarantine/Gatekeeper behavior. Signing/notarization acceptance applies if required by the selected Mac distribution path under [SECURITY.md](../../SECURITY.md); no release identity or signing process is selected by this proposal. The local generated C binary's signature proves none of these. No release identity or credential is acquired by the probe. |
| Release support | Publish the tested OS range, retain Linux x86_64 and macOS ARM64 native gates and full lifecycle observations. Prove editable local versus protected global instructions, this installation's complete protected set and safe overlapping locations; state that independent homes are outside the guarantee. Native verification is recorded in the linked test results. Unsupported or failed readiness has no weaker fallback. OS updates require revalidation; a deprecated interface can force a compatibility update or explicit refusal. |

Product admission, distribution and release support remain **first Flow Agent release gates**; [TESTING.md](../../TESTING.md#native-verification-status) distinguishes completed native checks from remaining acceptance. Historical feasibility, the prior native baseline and shared-platform tests cannot clear those gates. Do not select a release OS range solely from one hosted runner patch version. The [native comparison](#native-app-sandbox-comparison) establishes specific App Sandbox successes and restrictions, not arbitrary host-program compatibility. A passing feasibility matrix and an approved mechanism are not the claim that it is the only possible design or already release-ready.
