...
| Installed program objects | Current `flow`, the official sibling `flow-executor` and any explicitly selected Custom Executor, including every covered name of the same file. Reject missing or unresolved objects. The intentionally absent sibling in the existing Custom-only installation path needs the scope clarification in [D-068](../decisions/open-decisions.html#d-068) before implementing this admission rule. Custom code still belongs to the trusted installation, not third-party certification. |
