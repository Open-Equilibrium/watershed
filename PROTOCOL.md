...
## Accepted post-M1.1 target invariants

These invariants are approved but not implemented by the current M1.1 surfaces:
...
- Flow Agent resolves one Global Flow configuration authority and never discovers configuration from the current Workspace. Project-specific behavior is an explicit Flow, not a project-local config layer. Current `.flow/config.yaml`, workspace registry and automatic root `AGENTS.md` behavior predate this target; [D-057](docs/decisions/open-decisions.html#d-057) must define global storage, authoring and migration before implementation changes.
...
Flow Agent stores runtime state outside the Workspace in a private user-global home. `FLOW_AGENT_HOME` may select an absolute home; otherwise it is `$HOME/.flow-agent` on Unix and `%USERPROFILE%\.flow-agent` on Windows. Each canonical Workspace path maps to `workspaces/workspace-v1-<sha256>/`, where the digest uses a versioned domain plus the length-prefixed canonical native path bytes. Distinct canonical paths are distinct Workspace stores, including bind-mount aliases that remain distinct after canonicalization. Current M1.1 still reads Workspace configuration from `.flow/config.yaml`; ADR-0152 supersedes that configuration scope but not this implemented runtime-store contract. No implementation may claim Global Flow configuration until [D-057](docs/decisions/open-decisions.html#d-057) is resolved and the local discovery paths are removed.
