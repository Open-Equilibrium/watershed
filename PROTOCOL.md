...
**Flow security replacement:** The integrated invocation policy and Executor wire implement the [accepted contract](SECURITY.md#accepted-flow-agent-security-target), with native verification pending. Configuration administration remains manual in the first release. Removed isolation fields have no compatibility aliases and reject as unknown requirements. [TESTING.md](TESTING.md#m12-transition-and-executor-evidence) owns native status; shared-platform tests do not prove the Linux or Mac boundary.
...
The integrated official backends target `ubuntu-24.04-x86_64` (`bubblewrap-seccomp`) and `macos-26-aarch64` (`seatbelt`). These identifiers describe implemented selection, not completed native verification; [TESTING.md](TESTING.md#m12-transition-and-executor-evidence) owns the pending evidence.
