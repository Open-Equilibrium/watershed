...
- Request/preflight begins immediately before companion spawn. Publishing `Start` is bounded by the same existing Tool deadline plus the existing five-second Executor allowance; a fresh terminal window begins only after Flow Agent has written the complete matching `Start`, so Tool runtime never consumes preflight or control-delivery time.
