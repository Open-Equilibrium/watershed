...
flow import         := flow import LEGACY_WORKSPACE
...
`flow init`, `flow validate` and `flow create` operate only on the Global Flow home. `flow import LEGACY_WORKSPACE` is the sole legacy migration path: it explicitly reads exactly `LEGACY_WORKSPACE/.flow/config.yaml` and the registry that file selects beneath `LEGACY_WORKSPACE`, validates the complete source, and atomically publishes one previously absent Global Flow home. It never discovers another source, changes the source, imports `AGENTS.md`, replaces or merges a destination, or leaves a partial canonical destination.
