...
- Controller preparation also retains the selected Flow home and platform store and inventories their names and metadata, never credential/file contents. It counts distinct covered hardlink names across overlapping roots and rejects outside aliases, unsupported objects or incomplete inventory before Tool dispatch. The retained root directories own exclusive admission leases; Flow publishers take shared leases only around namespace mutations. Content append and whole Runs do not hold that lease. The scan occurs once per prepared controller, not per Tool; directory traversal is incremental, with identity bookkeeping for visited directories and multiply linked files. Native write protection, installed-program alias integration and complete admission performance/concurrency evidence remain unfinished; this check alone does not preserve the inventory against unconfined Tool code.
