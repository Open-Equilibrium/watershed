...
**Flow security transition:** Tool policy/schema/wire fields below describe the current legacy implementation. The approved replacement contract is in [SECURITY.md](SECURITY.md#accepted-flow-agent-security-target): ADR-0172 selects Mac Seatbelt and protected overlapping locations; ADR-0173 keeps configuration administration manual for the first release; ADR-0174 selects this installation's protected inventory. The replacement is not implemented. No new request fields or ignored legacy requirements are implemented here.
...
| Policy translation | `executor_policy_unsupported` | Executor rejects one requested canonical capability during preflight. |
| Sandbox preparation | `sandbox_setup_failed` | Backend failed after `Start` but before a proven Tool launch. |
