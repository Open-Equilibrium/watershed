...
**Flow security replacement:** The integrated invocation policy and Executor wire implement the [accepted contract](SECURITY.md#accepted-flow-agent-security-target). Configuration administration remains manual in the first release. Removed isolation fields have no compatibility aliases and reject as unknown requirements. [TESTING.md](TESTING.md#native-verification-status) owns native results and acceptance gaps; shared-platform tests do not prove the Linux or Mac boundary.
...
- Flow Agent resolves one Global Flow configuration authority from `FLOW_AGENT_HOME`, defaulting to `$HOME/.flow`. Its `config.yaml` and configured registry are the sole implicit technical configuration sources. Workspace `.flow/config.yaml`, Workspace registries and other ambient project configuration are never probed, merged or fallback sources; missing, invalid, inaccessible, unsafe or conflicting global state fails before Run/session mutation. Project-specific behavior is an explicitly selected Flow from the global registry.
...
- `tool.started` belongs to the active leaf Phase. Its progress and terminal event use the same flow, Phase and Tool identity. The retained Executor must first return `Ready`; Flow Agent commits `tool.started` and only then sends `Start` to that same process. A preflight `Error` occurs without `tool.started`, `Start` or a Tool result; after `tool.started`, exactly one terminal Tool event follows.
...
The field mapping is literal: `--id`/`--name` map to every block's `id`/`name`; hyphenated singleton flags map to the same snake-case field; singular repeat flags map to arrays (`--argv` to `command.argv`, `--instruction-ref` to `instruction_refs`, `--tool-ref` to `tool_refs`, `--phase-ref` to `phase_refs`, `--subflow-ref` to `subflow_refs`). Instruction parameter groups map to `parameters[]`; Tool parameter groups map to `allowed_parameters[]`. Loop and Transition files contain one bounded `ValuePredicate`; output and parameter files contain one closed `ValueContract`. For own-script Tools, the body maps to `script_body`; `command: script:<tool-id>` and `script_runtime: posix-sh` are derived. Removed filesystem/network/profile/capacity flags and fields are rejected, with no compatibility interpretation.

Group fields occur only inside their matching begin/end markers and in the displayed order, except that a Tool parameter's optional constraint flags may appear in any order after its required `name`, `value_type` and `required` fields. Type `none` permits no constraint flags; `string` requires exactly one pattern and maximum length; `enum` requires at least one allowed value; `integer` permits only optional minimum/maximum; `workspace-relative-path` permits only optional pattern/maximum length. A Flow requires at least one direct Phase. A leaf Phase has no child `phase_refs`, omits `result_from` and child Transitions, and may declare Instructions and Tools. A composite Phase has child `phase_refs`, declares exactly one `result_from` child with the same output contract, and declares no Instructions or Tools. A loop permits 1–32 iterations. Each Transition names two direct sibling Phases, points strictly forward, and its predicate is valid against the source output contract.
...
Preflight and policy rejection occurs before `tool.started`, creates no Tool result and retains the existing preflight-failure lifecycle. Companion launch, request parsing and preparation may precede that event; native restriction setup and Tool launch require matching `Start`. A validated productive Executor response produces exactly one terminal Tool event and synchronized terminal Run Log record. Dispatched intent without validated terminal evidence remains `uncertain`. The three evidence-only rows below belong solely to the gated direct-process M1.1 evidence runner; the productive Executor cannot emit them.
...
The exact operator commands are `flow auth login openai-codex <--browser|--device>`, `flow auth status openai-codex` and `flow auth logout openai-codex`. Flow Agent stores credentials at `$HOME/Library/Application Support/flow-agent/credentials.json` on macOS and `${XDG_CONFIG_HOME:-$HOME/.config}/flow-agent/credentials.json` on Linux; the resolved configuration base must be absolute.
...
Before every explicit provider attempt or provider-requested Tool call, Flow Agent synchronizes a durable attempt intent. A Tool intent also binds the expected applied-policy digest of the complete resolved invocation and protected inventory. A definitive Executor preflight rejection closes that intent and replays without `tool.started`, another preflight, `Start` or Tool redispatch. Cancellation after committed `tool.started` but before `Start` closes the Tool attempt with exact `{"schema":"flow-attempt-cancelled-v0"}` durable output; Resume can reconstruct a missing recovery record from that Run Log result without redispatch. After Flow Agent may have sent `Start`, missing or invalid terminal evidence leaves the intent `uncertain`; Resume never relaunches it automatically. Failure to commit `tool.started` closes the waiting Executor without `Start`, native restriction setup or Tool creation, while the already durable intent follows the same conservative uncertainty rule. `flow reconcile-tool <conversation-id> <run-session-id> --result <file|->` accepts one exact canonical `flow-tool-attempt-output-v1` evidence document containing exactly `schema`, `request_hash`, `enforcement` and `tool_result`. It validates the request hash and enforcement receipt against the uncertain intent, validates the `flow-tool-result-v0` value and referenced-object bounds, derives the only uncertain Tool attempt in that Run and appends its terminal result once. The command exposes no attempt id. Zero or multiple eligible attempts reject without Run mutation; provider attempts are not eligible. This provides at-most-once Flow Agent dispatch after intent, not exactly-once external effect. Agentic Engineers configure Building Blocks and their capability limits; other users may run predefined Flows without an additional Tool warning or authority-escalation surface.

## M1.2 Executor protocol (ADR-0146, ADR-0160, ADR-0161, ADR-0162)

The M1.2 Executor boundary applies to productive Tool execution. Provider transport remains inside Flow Agent. Flow Agent owns invocation validation, canonical policy compilation, Executor selection and lifecycle, durable attempt ordering, result validation and public errors. An Executor owns one request's native self-protection setup, Tool launch, bounded I/O, root-process/process-group supervision and enforcement receipt. The native backend supplies protected-write denial inherited by Tool children; it does not interpret Building Blocks or guarantee absolute hostile-descendant cleanup. The Agentic Engineer owns trust in the complete Tool/helper/library/service chain.
...
Each Executor configuration-store instance retains its first successfully opened parent directory. Reads, locks, staged publication, recovery and reset remain relative to that retained object; replacing its pathname does not redirect later operations. Reading an absent parent creates no configuration state. Installed-program validation likewise walks every directory component without following links and opens the image relative to the checked parent. These retained identities participate in admission; they do not replace native write enforcement or its required native evidence.

### Process and framing contract

- Flow Agent derives the default Executor only from its own resolved executable directory or uses the protected absolute Custom Executor override. It opens the selected executable without workspace search, `PATH` lookup, shell parsing or environment interpolation. Selection and Custom configuration also validate Flow itself and any present official sibling under [ADR-0175](docs/adr/ADR-LOG.md); only a genuinely absent optional sibling is omitted. Productive execution fails with `executor_unavailable` when a required installed object is absent or unsafe, or the selected object is replaced, incompatible or not ready; there is no fallback or provider/user-selected escalation. Admission alone is not native enforcement.
- Before every probe or preflight spawn on both Linux and macOS, reopen the selected path through the existing no-follow installed-program admission walk and compare its device/inode identity with the retained image. Missing, unsafe or mismatched paths reject before spawn. Linux starts the retained image through `/proc/self/fd`; Mac starts the checked native installation path. These mechanics share the [ADR-0176 installation-trust boundary](SECURITY.md#native-self-protection-and-its-limits); descriptor execution supplies no stronger public guarantee.
- Controller preparation retains the selected Flow home, platform store, required program images and any present official sibling, with their anchored parent directories. Before probing the Executor, one combined metadata inventory counts distinct covered hardlink names across directory roots and selected program names; it never reads credential/file contents or scans unrelated installed programs. Outside aliases, unsupported objects, replaced program names or incomplete inventory reject admission. Diagnostic/configuration checks inspect existing stores without creating an absent home; productive preparation first creates its required protected directories. The retained store roots own exclusive admission leases; Flow publishers take shared leases only around namespace mutations. Content append and whole Runs do not hold that lease. The scan occurs once per prepared controller, not per Tool; directory traversal is incremental, with identity bookkeeping for visited directories and multiply linked files. The integrated native backends must preserve that admitted inventory against Tool/child writes. Cooperative leases alone do not constrain unconfined code.
- Flow Agent starts one fresh companion process for one operation. It writes exactly one `flow-executor-request-v0` document, reads one flushed `flow-executor-preflight-v0`, and retains that same process. `Ready` permits Flow Agent to commit `tool.started` but grants no execution authority. Flow Agent then writes exactly one matching `flow-executor-start-v0`, closes stdin and reads one terminal `flow-executor-result-v0`. A preflight `Error` is terminal without `Start` or a result. `<executor> --probe` returns one `flow-executor-probe-v0` document. The companion has no daemon, listening socket, persistent per-Flow boundary, pooled guest or remote transport.
- The process environment starts empty. Flow Agent keeps stdin open through `Ready`, then closes it after `Start`; it drains stdout and stderr concurrently under the Tool-runner byte and time bounds and treats stderr as bounded redacted diagnostics rather than protocol data. EOF, cancellation, malformed or oversized input, or a mismatched `Start` before execution authority aborts without native restriction setup or Tool creation.
- Preflight, `Start` publication and terminal handling each receive a fresh independent window of `timeout_ms` plus the five-second Executor allowance. The preflight window begins immediately before companion spawn. The `Start` window begins only after `Ready` and the successful durable `tool.started` commit, when Flow Agent begins publishing `Start`. The terminal window begins only after Flow Agent has written the complete matching `Start` and closed stdin, so no earlier stage consumes a later stage's time.
- All request, preflight, Start, result and probe schemas are closed, reject duplicate members, unknown versions, excess nesting and non-canonical integers, and use the ordinary protocol JSON parser limits. Each stage permits exactly its required canonical document and LF. Premature exit, a missing or unexpected stage, extra documents or trailing non-whitespace, malformed/oversized output, or any request-id mismatch is `executor_invalid_response`.

`flow-executor-request-v0` contains exactly `schema`, opaque per-attempt `request_id`, `policy_digest` and `resolved_policy`. The resolved policy is:

| Field | Meaning |
|---|---|
| `executable`, `argv` | Prevalidated absolute executable and literal argument vector; an argument-free invocation uses `[]`. |
| `environment` | Exact admitted name/value map; no ambient inheritance or Flow-owned provider credentials. |
| `working_directory` | Canonical absolute host directory. |
| `tool_id`, `tool_kind` | Validated Tool identity and kind. |
| `limits` | Exactly `{timeout_ms, max_stdout_bytes, max_stderr_bytes}`, all positive and subject to protocol bounds. |
| `protected_objects` | 1–128 records, each exactly `{descriptor, path, identity}`; `identity` is `{device, inode, kind}`, where `kind` is `file` or `directory`. |

Protected-object paths are unique canonical absolute host paths. Records use sequential inherited descriptor slots starting at 32: record `i` uses `32 + i`, so the full range is FD 32–159. These descriptors carry already-open objects; the Executor verifies descriptor and path identity during preparation and again before execution. Flow chooses the [complete protected inventory](docs/concept/flow-agent-native-protection-proposal.md#complete-protected-set). Unintended descriptors, writable protected handles and terminal authority must not reach the Tool.

`policy_digest` is lowercase SHA-256 over the canonical JSON serialization of the complete `resolved_policy` plus its final LF. It binds the executable, argv, environment, working directory, Tool identity, limits and protected objects. The request carries neither a second policy-byte field nor old mount, network, runtime-profile or process-capacity fields. It represents one invocation and carries no transcript or authority supplied by model output.

`flow-executor-preflight-v0` returns schema, matching `request_id` and an `outcome` of `ready` or `error`; errors also carry their stable code and bounded message. `flow-executor-start-v0` contains exactly schema and matching `request_id`; any other or absent control record denies execution. `Ready` and `Start` denote these protocol stages, with no compatibility fallback.

After `Start`, `flow-executor-result-v0` carries schema, matching `request_id` and either `outcome: "completed"` with `tool_result` and `enforcement`, or `outcome: "error"` with `sandbox_setup_failed` before a proven Tool launch. A completed envelope means terminal evidence is present, not necessarily Tool success. Its binary-safe `tool_result` has status, classification, optional numeric exit evidence and canonical padded-base64 stdout/stderr; Flow validates it and converts it to the public `flow-tool-result-v0` value above.

The enforcement receipt contains exactly `executor`, `executor_version`, `backend`, `backend_version`, `platform`, `applied_policy_digest` and `self_protection_active`. Flow requires a matching digest, the selected Executor/backend/platform identity and `self_protection_active: true` on every terminal execution response. It persists the validated canonical receipt with the terminal attempt before publishing success. Missing, mismatched or inactive evidence fails closed; after dispatch it leaves an uncertain effect. The receipt attests the requested write protection, not general isolation, descendant cleanup or truthful Tool results. A Custom Executor can lie; structural validation is not certification.

`flow-executor-probe-v0` reports schema, protocol versions, Executor/backend identity and versions, exact platform, supported policy features and Boolean `ready` from a no-Tool-spawn readiness check. The current required feature is `flow-owned-write-protection`; there are no runtime-mount manifests or per-Tool profiles. Flow probes once per prepared controller before durable productive Run reservation; failure creates no Run. `flow executor check` uses the same diagnostic probe. A passing probe is advisory; each invocation still validates identities and its receipt, and native tests must establish the actual boundary.
...
| Protocol validation | `executor_invalid_response` | Malformed, oversized, mismatched, unexpected or incomplete stage/evidence. |
| Policy validation | `executor_policy_unsupported` | Executor cannot apply the requested invocation/protected-object contract during preflight. |
| Native protection preparation | `sandbox_setup_failed` | Backend failed after `Start` but before a proven Tool launch. |
| Tool lifecycle | Tool classifications | Self-protection was active; the Tool root exited, timed out, was cancelled or failed bounded output handling. This is not complete descendant-cleanup evidence. |
...
Mac invokes Apple's `/usr/bin/sandbox-exec` with a narrow Seatbelt policy: allow ordinary host access, deny writes to protected files/directories, deny ancestor unlink/move and direct terminal-device access. Its inner Executor also starts from the checked native installation path under [ADR-0176](SECURITY.md#native-self-protection-and-its-limits); detected identity changes reject before that start without fallback. Paths are bound as parameters rather than interpolated profile source. The accepted deprecated-interface/OS-update risk requires native revalidation.
...
- `tool.started`: `tool_id`, `tool_name`, `tool_kind` (`predefined-command | own-script`), `allowed_parameters` (string array of allowed parameter names), plus `attempt_id` for productive Tool calls. Mounts, runtime profiles, network access and process/thread capacity are not payload fields.
