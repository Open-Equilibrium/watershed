...
- Request/preflight and Start/terminal are separately bounded by the existing Tool deadline plus the existing five-second Executor allowance. The first window begins at companion spawn; the second begins when Flow Agent sends `Start`, so Tool runtime never consumes the preflight window.
