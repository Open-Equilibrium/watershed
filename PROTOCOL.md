...
**Flow security transition:** Tool policy/schema/wire fields below describe the current legacy implementation. The approved replacement contract is in [SECURITY.md](SECURITY.md#accepted-flow-agent-security-target): ADR-0172 selects Mac Seatbelt and protected overlapping locations; ADR-0173 defers ADR-0168 Tool-initiated configuration review until after the first Flow Agent release, which retains manual administration only. The replacement is not implemented; [D-063](docs/decisions/open-decisions.html#d-063) retains the protected-inventory choice. No new request fields or ignored legacy requirements are implemented here.
