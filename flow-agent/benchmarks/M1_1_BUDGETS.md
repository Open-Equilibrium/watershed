...
- `P:<name>` is a release-mode, one-process-at-a-time benchmark on the fixed `ubuntu-24.04` x64 runner. Five warmups and 30 measured fresh-child samples produce `target/m11-budgets/m11-budgets.jsonl` with schema `flow-m11-budget-v0`. Timing gates compare the measured p95; RSS gates compare the maximum unadjusted `post-workload VmHWM - pre-workload VmRSS`. A gate passes only when every applicable aggregate is at or below its selected limit.
...
| CV-12 | 16 MiB migration/replay/history-validation quantum: p95 1 s | Quantum behavior is covered by CV-03/CV-04/CV-08. | `P:conversation_migration_quantum`, `P:conversation_replay_quantum` and `P:conversation_history_validation_quantum` each process exactly 16 MiB, each p95 <= 1 s and subject to CV-09. |
...
| CV-17 | Callback-streaming full-Run replay: exactly 352 MiB across 22 segments, p95 10 s | `F:streaming_replay_emits_large_segmented_jsonl_without_returning_it` proves validated record streaming, byte identity and empty returned output above CV-16. | `P:conversation_full_run_streaming_replay` validates and hashes exactly 369,098,752 bytes across 22 segments, p95 <= 10 s. |
