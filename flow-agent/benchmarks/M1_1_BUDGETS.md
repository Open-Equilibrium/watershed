...
| CV-12 | 16 MiB migration quantum: p95 1.6 s; replay/history-validation quantum: p95 1 s | Quantum behavior is covered by CV-03/CV-04/CV-08. | `P:conversation_migration_quantum`, `P:conversation_replay_quantum` and `P:conversation_history_validation_quantum` each process exactly 16 MiB and are subject to CV-09. Migration p95 <= 1.6 s; replay and history-validation p95 <= 1 s. |
...
| CV-17 | Callback-streaming full-Run replay: exactly 352 MiB across 22 segments, p95 15 s | `F:streaming_replay_emits_large_segmented_jsonl_without_returning_it` proves validated record streaming, byte identity and empty returned output above CV-16. | `P:conversation_full_run_streaming_replay` validates and hashes exactly 369,098,752 bytes across 22 segments, p95 <= 15 s. |
