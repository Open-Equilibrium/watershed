...
expected_path = Path("expected-environment.json")
if sys.argv[1:] == ["baseline"]:
    # The controller launches this exact runtime with env_clear + the request
    # environment. Any further entries are generated by the shell/runtime, not
    # inherited authority (e.g. Darwin's __CF_USER_TEXT_ENCODING).
    with expected_path.open("x") as output:
        json.dump(dict(os.environ), output)
    raise SystemExit(0)
expected = json.loads(expected_path.read_text())
assert dict(os.environ) == expected, sorted(
    key for key in set(os.environ) | set(expected)
    if os.environ.get(key) != expected.get(key)
)
...
"#,
    ...
    let baseline = fixture.request(
        "exec python3 exercise.py baseline",
        limits(1024, 4096, 5000),
    );
    fixture.baseline(&baseline, &[]);
    let expected = fs::read(fixture.project.join("expected-environment.json")).unwrap();
    let result = fixture.run("exec python3 exercise.py", limits(1024, 4096, 5000));
    native_support::assert_success(&result);
    assert_eq!(
        fs::read(fixture.project.join("expected-environment.json")).unwrap(),
        expected
    );
    assert_eq!(
