...
```bash
"$AUTOREVIEW" --reviewers codex
```

`--panel` is shorthand for a Codex/Claude panel unless `--engine` changes the first reviewer:
...
- supports `--engine codex`, `claude`, `droid`, and `copilot`; Codex remains the repo's normal closeout engine unless explicitly requested
