...
8. **MVP VCS boundary.** Do not add **project-code** VCS/history-engine behavior (version control over arbitrary software projects, e.g. Git/Jujutsu-style commit/branch/history management) to Loop Agent, Meta-Harness or Liquid. The MVP works inside normal Git projects; project-code VCS/history questions are deferred until after the MVP. This does **not** restrict Liquid's internal **workspace action history / workspace VCS** over its own workspace data (dashboards, views, components, scripts, sources, settings), which is an in-scope Liquid product responsibility — see `docs/concept/V-Spec_Liquid.html`.
...
- Subagent liveness: before treating any subagent as stale, send it a direct status message and wait for a reply. For `autoreview_runner`, if it does not respond for 30 minutes after the status request and no commits were added during that window, close it as stale; if commits or dirty files appear, keep coordinating instead.
