...
ADR-0167 authorizes a bounded macOS feasibility evaluation of a native sandbox-profile mechanism without a stable public Apple interface contract. This is an evaluation-only exception to the no-private-API-debt constraint, not permission to ship it. Compare native protection and ordinary host-program compatibility with documented, supported Apple facilities; report OS-update maintenance risk and installation/signing evidence before asking the maintainer to select a shipping mechanism. No native test result or product support is implied by this authorization.
