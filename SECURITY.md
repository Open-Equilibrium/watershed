...
**ADR-0166–ADR-0175 define the integrated replacement; ADR-0176 settles installation trust and launch.** This section owns the Flow Agent security contract. Runtime, invocation policy and Executor wire use trusted Tools with mandatory native self-protection on Linux and macOS. [TESTING.md](TESTING.md#native-verification-status) owns native CI results and remaining acceptance gaps. Integration and passing Windows shared tests do not establish native protection or release readiness.
...
Building Blocks, model output and Workspace-local files cannot select or replace an Executor. Missing or failed admission/readiness prevents launch without fallback. Custom evidence is structurally checked, not certified as truthful. Fixture execution remains deterministic emulation. [PROTOCOL.md](PROTOCOL.md#m12-executor-protocol-adr-0146-adr-0160-adr-0161-adr-0162) owns the wire and backend mechanics; [TESTING.md](TESTING.md#native-verification-status) owns native proof and its limits.
...
Installation distributes one version/platform-specific `.tar.gz` containing prebuilt artifacts, the installer, version/platform metadata and license, plus `SHA256SUMS`, rather than `.deb` packages. Users download through the browser and verify the selected archive with existing OS tools before extraction or execution; no online bootstrap is included. Ordinary dependencies are explicit. Missing native mechanisms or incompatible host security policy must produce an actionable failure, not automatic kernel upgrades, AppArmor/sysctl changes or service activation. Administrators decide host-wide changes separately. The replacement has no systemd user-manager or delegated-cgroup prerequisite; product execution remains unprivileged.
