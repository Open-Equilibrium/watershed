...
**ADR-0166–ADR-0172 are approved, not implemented.** This section owns the replacement Flow Agent contract. Existing M0/M1 policy artifacts, M1.2 wire fields, Ubuntu enforcement and their tests remain the **legacy implementation**, not the future release promise. They must remain functional until a coherent, tested migration replaces them. Native macOS Tool execution remains unavailable. ADR-0172 selects the Mac Seatbelt profile mechanism; [D-063](docs/decisions/open-decisions.html#d-063) owns the remaining protected-inventory choice. Mechanism approval is not completed product integration or release evidence.
...
| Flow-owned files | A mandatory native boundary blocks direct Tool/child writes, deletion and replacement of protected Flow configuration, registry, runtime/credential stores and installed program objects. Tools cannot choose the protected set or disable it. Missing protection fails before Tool launch. | Do not treat this as whole-host isolation. Privileged actors, controller/OS compromise and effects delegated to independent unconfined services are excluded. File/object discovery and inherited protection require native proof; [D-063](docs/decisions/open-decisions.html#d-063) selects only additional independent-home coverage. |
| Configuration changes | Only Flow Agent applies validated, concrete change requests under the configured `deny` (default), `ask` or scoped `allow` policy. `ask` requires an authorized person's approval of that exact change; no answer grants no authority. | Engineers choose the permitted change scope and whether a person may approve within it. Tools submit proposals, never unrestricted file edits. |
| Authority integrity | A Tool cannot approve itself, widen its own permission, replace enforcement code or silently change the authority of the current Run. Configuration permission never grants arbitrary edits to credentials, installed programs or execution history. | Administrators remain responsible for trusted installation and deliberate out-of-band changes. An approval is not transferable authority for later unrelated requests. |
...
ADR-0167 authorized the bounded native feasibility evaluation. After comparison with App Sandbox, ADR-0172 selects Seatbelt through `/usr/bin/sandbox-exec` for the macOS ARM64 release backend, with the stated deprecated-interface and OS-update maintenance risk accepted. Implement only the narrow Flow-owned direct-write/review-channel boundary, not a general filesystem/network permission product. Preserve checked admission, inherited protection and fail-before-launch behavior; OS updates require revalidation. This approval introduces no VM, permanent privileged service, new dependency, copied upstream permission engine or weaker fallback. Actual integration, supported-version and installation/signing evidence remain release gates in [TESTING.md](TESTING.md).
...
Configuration writes always use the controlled request path, including under `allow`; direct protected writes remain blocked. An OS-denied write does not become an approval request, and Flow must not rerun a partially effective Tool to replay it with greater authority. Review shows the exact proposed change without exposing credentials. No approval may silently authorize a changed proposal or override a conflicting configuration version.

ADR-0167 limits the initial request catalog to the existing Global Flow configuration fields `model`, `model_context_limit` and `output_reserve`, subject to their existing validation and the Building Block's allowed scope. `ask` uses local-owner review; delegated and remote reviewers are outside this initial scope. Registry paths, executable selection, Tool permissions, credential records, installed programs and all other fields remain outside the automated catalog and require manual administration.

ADR-0168 selects the following initial review lifecycle; it is not implemented:

1. Process an explicit structured proposal after its Tool completes; human waiting never extends the Tool deadline. Dispatch no further provider or Tool work in the requesting Run while review is pending.
2. Accept `ask` only in an explicitly interactive human Run. Flow itself shows the submitting Tool, affected Flow home and exact old/new values through a controller-owned terminal review channel. JSONL, redirected-input and unattended Runs reject `ask` immediately; use preconfigured scoped `allow` or manual administration there.
3. Allow five minutes to respond. Refusal, timeout, cancellation or channel loss rejects the proposal and ends the requesting Run without a change or further dispatch. Keep no pending approval across restart; audit evidence is not retained authority. A later proposal needs a new explicit invocation.
4. Bind one-use consent to the local owner, Tool/Run, exact proposal, Flow home and unchanged configuration version. Revalidate scope and the complete resulting configuration immediately before atomic publication. A concurrent edit invalidates consent; never overwrite or silently merge it. Tool output is never an answer.
5. Successful changes affect future Runs only, not configuration or authority captured by existing Runs. An interrupted publication may be uncertain; never automatically repeat it or reuse its approval.

A terminal or same-user identity alone does not authenticate consent against Tool code. The native implementation must prevent direct Tool/child access to the review channel and inherited approval handles; terminal display must distinguish and safely render untrusted output. Independent automation services and controller/OS compromise retain the exclusions above. Channel protection, transaction conflicts and crash behavior require real acceptance tests before enabling `ask`; a feasibility probe is not that implementation.

Migrate the checked-in Flow definitions, schema, policy/protocol, goldens and tests together. Unsupported legacy isolation requirements must be explicitly rejected before effects, never parsed as ignored security settings. Existing tests are evidence for current behavior, not proof of the new boundary; the replacement requires red-first behavior tests and native evidence on both release targets. Standard Tool scope and trust UX must be decided before the first Flow Agent release under [D-066](docs/decisions/open-decisions.html#d-066). A Building Block marketplace and its specification are later-roadmap work under [D-067](docs/decisions/open-decisions.html#d-067), not a certification promise.
...
The Flow-specific broad-isolation clauses below describe the current legacy implementation pending ADR-0166; the accepted replacement is [above](#accepted-flow-agent-security-target). Other products' contracts remain normative without that qualification.
...
The current legacy M1.2 implementation separates policy orchestration from isolation mechanics. Its broad restrictions below are not the ADR-0166 release target:

1. Before durable productive Run reservation, Flow Agent validates the selected administrator-owned Executor and backend readiness. For each Tool call it validates the request against the selected Building Blocks, compiles the canonical policy and synchronizes durable intent. The same one-shot Executor must return `Ready`; Flow Agent commits `tool.started` and only then sends the matching `Start`.
2. The Executor understands the versioned Flow request, rejects unsupported policy during preflight, and waits after `Ready` without constructing a Sandbox or Tool. Only `Start` authorizes it to translate the exact policy to one Sandbox backend, own one Tool process tree and return a canonical enforcement receipt bound to the exact applied-policy digest.
...
The prebuilt installation path remains unimplemented. Before the first Flow Agent release, prove clean native installation without compilation or extra developer tools: verify the selected version, platform and final bundle checksum before executing its payload; reject failed HTTPS verification, missing verification data, altered or wrong-target packages; cover every shipped executable and installer byte in the final checksum after any Mac signing/notarization/stapling. Retain the [create-only installer boundary](README.md#developertest-installation-on-ubuntu-2404-x64): install only into the intended unused prefix, reject unsafe paths, expose required privileges/dependencies, and recover only this attempt's own files on failure. This adds neither in-place upgrades nor an automatic updater. Internal development and publication safeguards must also be decided and verified before Release 1 under [D-065](docs/decisions/open-decisions.html#d-065); documentation or ordinary branch CI alone is not evidence that those safeguards are active.
...
Agentic Engineers configure Building Blocks and their capability limits, including each Tool's positive concurrent process-and-thread capacity; other users may run those predefined Flows but cannot widen them. Flow Agent neither warns nor asks for confirmation before an assigned Tool runs. The Executor confines Tool processes and descendants; provider authentication remains Flow-owned and is never forwarded in an Execution request. Before provider or Tool dispatch, Flow Agent synchronizes durable intent. For Tool execution, `Ready` grants no launch authority and matching `Start` is the external-effect boundary: pre-Start EOF, cancellation or commit failure cannot create a Sandbox or Tool, while missing terminal evidence after `Start` is uncertain and never retries automatically. The exact Tool reconciliation command is defined in `PROTOCOL.md`.
...
This section documents the current legacy M0/M1/M1.2 behavior. The accepted replacement is defined above, not by these old isolation fields.

1. Compile script policy → deterministic fixture evaluation plus canonical OS policy artifacts. The official Executor maps exact pre-opened mount capabilities to stock Bubblewrap namespaces/mounts plus seccomp on Ubuntu 24.04 x64. Official productive macOS execution remains fail-closed until its native boundary is implemented and verified under ADR-0163; [D-063](docs/decisions/open-decisions.html#d-063) selects only whether first release covers additional independent Flow homes. No supported path degrades to a weaker or unsandboxed backend.
2. **Network egress deny-by-default**. M1 performs no real network access, rejects non-empty Linux-target CIDR allow entries and emulates deny-all decisions for negative fixtures (ADR-0051, ADR-0052). M1.2 isolates Tool networking and supports deny-all only; positive grants are unsupported by this legacy implementation. General Tool network enforcement is not part of the ADR-0166 target.
...
5. **Runtime reads and lifecycle.** `runtime_profile: exact` is the default and exposes only the readiness-advertised bounded executable/interpreter/library objects. `host-system-read` is an explicit Agentic Engineer choice that adds only the Executor's fixed reviewed read-only Ubuntu system roots. Flow pre-opens every advertised source and binds its identity and Sandbox target into the policy digest; the Executor cannot add undeclared paths. Flow users, providers and Tools cannot alter or escalate the profile, and no automatic fallback exists. The statically linked official Executor needs no broad bootstrap runtime; its trusted inner process starts from an inherited self-image descriptor that is closed to the Tool, and a dynamic official artifact fails readiness. Each invocation receives a fresh transient systemd scope and cgroup-v2 leaf whose `pids.max` counts only the Tool root, descendants and threads. The required configured value is bound into policy, request, receipt and recovery. The Executor bounds I/O and time, terminates the complete process tree, validates enforcement and empty-leaf cleanup, and persists the receipt with the terminal Tool attempt. Missing user-manager, delegation, capacity-event or cleanup support fails readiness; there is no persistent per-Flow Sandbox or weaker fallback. Exact I/O and time limits remain canonical in the M1.1 budget matrix.
...
- `commands[].filesystem`: exact `read_only_mounts` and `writable_mounts` string arrays after registry resolution.
- `commands[].max_concurrent_processes_and_threads`: required positive Tool-process and thread limit, including descendants.
- `commands[].runtime_profile`: `exact` by default or explicit `host-system-read`. The M1.2 translation and descriptor rules are canonical in the [Executor protocol](PROTOCOL.md#m12-executor-protocol-adr-0146-adr-0160-adr-0161-adr-0162).
...
V0 network policy grammar is IP/CIDR based. A future isolated tool may not rely on the policy compiler to resolve hostnames. The `linux-bubblewrap-seccomp` policy rejects non-empty allowlists (ADR-0051); fixture execution performs no DNS, DoH or DoT traffic. This is a current legacy artifact grammar, not a planned general-egress guarantee under ADR-0166.
