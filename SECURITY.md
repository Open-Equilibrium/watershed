...
Registry loading starts from one opened workspace capability, descends every registry directory without following links and reads each YAML leaf from the same no-follow handle. Linux and macOS are the primary targets; the private boundary remains portable to Windows (ADR-0063, ADR-0064).
