...
| Configuration changes | The first Flow Agent release provides no Tool-initiated configuration-change path. Direct protected writes remain blocked; no permission prompt or automatic retry lifts the block. | Administrators change configuration manually outside Tool execution. Scoped proposals and their approval workflow are deferred below, not first-release capabilities. |
...
#### After the first Flow Agent release: configuration proposals

ADR-0173 defers the following accepted catalog and workflow, including their protocol, review UI and feature-specific acceptance tests. They are not requirements for the first Flow Agent release or a promise of a particular later version.

Tool-initiated configuration changes always use the controlled request path, including under `allow`; direct protected writes remain blocked. An OS-denied write does not become an approval request, and Flow must not rerun a partially effective Tool to replay it with greater authority. Review shows the exact proposed change without exposing credentials. No approval may silently authorize a changed proposal or override a conflicting configuration version.
