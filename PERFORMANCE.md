...
The one-shot Executor and Sandbox architecture is canonical in `PROTOCOL.md`. ADR-0158 makes that architecture target, rather than an estimated absolute latency number, the M1.2 performance contract. The fixed Ubuntu 24.04 x64 job retains observational regression evidence; other platforms retain the functional-test policy above, and Custom Executor performance remains administrator-owned.

The dedicated [M1.2 startup evidence](flow-agent/benchmarks/M1_2_BUDGETS.md) is separate from the closed M1.1 matrix. Each fresh child measures one direct Tool invocation, reports the enclosing runner lifecycle and independently reported no-op Tool runtime as unadjusted distributions, and performs no subtraction. CI fails on invalid lifecycle, schema, bounds, report completeness or missing evidence, but not on an arbitrary timing estimate. Performance remains a review KPI, and cleanup or isolation cannot be traded for latency.
