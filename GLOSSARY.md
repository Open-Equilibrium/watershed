...
- **Execution request** — One versioned, bounded request from Flow Agent to an Executor for exactly one validated Tool invocation. It carries the verified executable, arguments, working directory, environment, exact mount descriptor slots, runtime-read profile, process/thread capacity, fixed runtime limits and canonical policy digest; it grants no authority beyond that policy.
- **Executor** — A Flow Agent execution component that validates and translates one Execution request, manages one Tool process tree and returns a bounded result or typed error. Successful results carry a canonical enforcement receipt.
- **Default Sandbox Executor** — The official statically linked Ubuntu `flow-executor` sibling installed by the standard M1.2 path. It maps Flow policy to stock Bubblewrap plus seccomp and fails closed when that boundary is unavailable or not ready.
...
- **Runtime-read profile** — The named read-only runtime capability set selected by Tool field `runtime_profile` and bound into its policy. `exact` is the default readiness-advertised executable/interpreter/library set; `host-system-read` is an explicit Agentic Engineer choice that adds only fixed reviewed Ubuntu system roots.
- **Tool process capacity** — The required positive `max_concurrent_processes_and_threads` capability for one Tool invocation. It counts the Tool root, descendants and threads inside one fresh cgroup leaf; trusted Executor and Sandbox supervisor processes do not count.
...
- **Sandbox** — The fresh restricted working environment created for one untrusted Tool invocation. The term describes the security boundary, not the component that interprets Flow Agent rules.
...
- **flow-m12-executor-startup-v0** — Release-mode Ubuntu JSONL schema for the fixed M1.2 one-shot Executor startup observation; its canonical workload, fields and evidence policy live in [`M1_2_STARTUP_EVIDENCE.md`](flow-agent/benchmarks/M1_2_STARTUP_EVIDENCE.md), without a timing threshold.
