...
- **Execution request** — One versioned, bounded request from Flow Agent to an Executor for exactly one validated Tool invocation. It carries the verified executable, arguments, working directory, environment, exact mount descriptor slots, runtime-read profile, limits and canonical policy digest; it grants no authority beyond that policy.
