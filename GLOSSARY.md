...
- **Flow Agent** — Host-local, CLI-only, Rust-core, script-driven deterministic Flow runtime (not a generic coding agent). M1 is fixture-bounded; the current M1.2 implementation has legacy OS isolation. The approved replacement security target is canonical in [SECURITY.md](SECURITY.md#accepted-flow-agent-security-target). CLI binary: `flow` (ADR-0013).
...
- **Tool** — A Phase-scoped capability with an exact command identity (predefined or own script), parameters, read-only/writable mounts, runtime-read profile and network boundary. Referencing it only makes it available; the provider decides whether and how often to request it but cannot widen its authority.
- **Policy artifact** — The canonical JSON output produced by `core-policy` to bind one Tool's resolved capabilities and runtime-read profile to a target enforcement backend. Fixture evaluation proves deterministic decisions but provides no OS security boundary.
...
- **Predefined-command Tool** — A Tool that names a fixed command id resolved through the predefined-command registry. Fixtures emulate its result; productive execution sends the verified identity through the Executor boundary.
...
- **Own-script Tool** — A Tool whose reviewed inline `script_body` uses the fixed v0 `posix-sh` contract. Fixtures interpret only their bounded subset; productive execution uses the same Executor boundary as every other Tool.
...
- **Execution request** — One versioned, bounded request from Flow Agent to an Executor for exactly one validated Tool invocation. The current legacy wire form carries the verified executable, arguments, working directory, environment, exact mount descriptor slots, runtime-read profile, process/thread capacity, fixed runtime limits and canonical policy digest; it grants no authority beyond that policy.
- **Executor** — A Flow Agent execution component that validates and translates one Execution request, supervises one Tool invocation and returns a bounded result or typed error. The current legacy wire contract includes an enforcement receipt; observed cleanup is not a universal hostile-descendant guarantee.
- **Default Sandbox Executor** — The official `flow-executor` companion shipped by default. Current legacy Ubuntu execution uses Bubblewrap/seccomp; the approved native replacement must establish self-protection before Tool launch. Exact release targets are in [PLATFORMS.md](PLATFORMS.md).
- **Custom Executor** — An administrator-selected implementation of the Executor protocol on the supported Ubuntu 24.04 x64 M1.2 platform. Flow Agent documents the protocol and reports integration failures, but does not certify or guarantee a third party's compatibility, security or operation.
...
- **Runtime-read profile** — The current legacy read-only runtime capability set selected by Tool field `runtime_profile` and bound into its policy. `exact` is the default readiness-advertised executable/interpreter/library set; `host-system-read` is an explicit Agentic Engineer choice that adds only fixed reviewed Ubuntu system roots.
- **Tool process capacity** — The current legacy positive `max_concurrent_processes_and_threads` capability for one Tool invocation. It counts the Tool root, descendants and threads inside one fresh cgroup leaf; trusted Executor and Sandbox supervisor processes do not count.
- **Enforcement receipt** — The canonical bounded Executor evidence persisted with a terminal Tool attempt. It identifies the applied backend/platform and binds the exact canonical policy digest; structural validation does not certify a Custom Executor's truthfulness.
...
- **Sandbox** — An execution environment whose selected restrictions are enforced by an operating-system, container or VM mechanism. The term alone promises neither whole-host containment nor a particular set of restrictions; Flow's accepted narrow target is defined in [SECURITY.md](SECURITY.md#accepted-flow-agent-security-target).
- **Sandbox backend** — The operating-system, container or virtual-machine mechanism that constructs and enforces a Sandbox. Stock Ubuntu Bubblewrap plus seccomp is the official M1.2 backend; a backend does not understand Building Blocks by itself.
...
- **Configuration change request** — A concrete proposal that only Flow Agent may validate and apply under an Engineer-configured deny/ask/scoped-allow rule. It never gives a Tool direct write access to protected files; the implementation contract remains D-063.
...
- **flow-m11-performance-evidence-v0** — Release-mode Ubuntu JSONL schema for the finite M1.1 observational workload set; it records exact inputs, raw samples and unadjusted timing and RSS aggregates without performance thresholds.
- **flow-m12-executor-startup-v0** — Release-mode Ubuntu JSONL schema for the fixed M1.2 one-shot Executor startup observation; its canonical workload and evidence policy live in [`M1_2_STARTUP_EVIDENCE.md`](flow-agent/benchmarks/M1_2_STARTUP_EVIDENCE.md), without a timing threshold.
...
- **Conversation entry** — One `flow-conversation-entry-v1` history node that links to its parent and to a committed point in one owned linear Run, and binds the terminal recovery snapshot used for compact continuation.
