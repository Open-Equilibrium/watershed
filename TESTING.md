...
- **M1.2 Executor and OS-isolation tests** — protocol conformance and both installation paths are platform-independent; the official Default Sandbox Executor runs the canonical hostile matrix with stock Bubblewrap/seccomp in the pinned Ubuntu 24.04 x64 contract environment inside the Ubuntu CI job. Its outer network remains available so only the inner Executor denial can satisfy hostile network tests. Productive Tool execution on macOS and Windows fails closed, and positive network grants are post-MVP. No test result certifies a Custom Executor.
...
- **Installation and readiness:** one acceptance test installs the standard Ubuntu package and resolves its administrator-owned sibling `flow-executor` with empty `PATH` and an arbitrary working directory. A second Ubuntu test uses the explicit `--no-default-executor` path, proves authoring, validation and Fixture execution still work, then proves a productive Tool Flow returns `executor_unavailable` before durable Run reservation or provider contact until an absolute Custom Executor override is configured. Core Flow tests prove readiness precedes reservation; the real official-Executor matrix and startup evidence prove productive dispatch and receipt validation. Unsafe, absent, replaced or incompatible siblings and overrides fail before Run creation.
- **Official boundary:** Ubuntu 24.04 x64 runs the hostile black-box matrix against real Tool descendants. It covers exact read-only/writable mounts, symlinks, hardlinks and replacement races, both runtime-read profiles, interpreter and environment escape, credential reads, direct/indirect/DNS network attempts, process/session escape, descendants, timeout, cancellation, terminal-status integrity, output handling and teardown. The real matrix executes the descriptor-mount capability advertised by stock Ubuntu Bubblewrap; deterministic plan tests cover both native descriptor mounts and the supported `/proc/self/fd/<slot>` compatibility translation. Canonical-policy and applied-policy digests must match.
...
- **Receipt and recovery:** every successful execution returns a canonical enforcement receipt bound to the exact canonical policy bytes including final LF; Flow Agent rejects a missing or mismatched receipt and persists the validated receipt with the terminal Tool attempt. Recovery and replay prove that evidence remains bound to that attempt without redispatch.
- **Fail closed:** missing prerequisites, incompatible protocol, unsupported capability, failed Sandbox preparation and failed readiness never select a weaker backend. Official productive macOS and Windows execution reject before Tool spawn; M1.2 Tool networking remains deny-all.
...
- **Platform scope:** Ubuntu 24.04 x64 is the only official productive M1.2 release gate. macOS is a fail-closed negative gate pending the post-M1.2 review in ADR-0160. Windows, other OS/version combinations and positive CIDR/port grants have no support claim.
