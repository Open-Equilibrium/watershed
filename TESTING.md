...
- **Performance gates** — benchmarks assert the `PERFORMANCE.md` targets/budgets relevant to the current milestone. M1 Loop Agent benchmarks run optimized in CI, measure per-event distributions (not batch-average distributions), exercise append, producer-side notification attempts and 10 concurrent near-limit registry closures, and exclude model latency, tool runtime, checkpoint synchronization, caller replay and caller transport. Failures block release.
