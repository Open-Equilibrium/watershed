...
- **Context compiler tests (Loop Agent)** — `loop-context-v0` tests assert Tier 0 source/order completeness, active-scope filtering, whole-unit Tier 1 selection/eviction, paired tool call/results, deterministic artifact projections, cache-prefix boundaries, budget/estimator behavior, fail-before-provider errors, canonical context hashes/manifests and byte-identical reconstruction on resume. Tests also prove Tier 2/3 sources remain in durable history while omitted from provider context (ADR-0058).
...
- **Sandbox boundary tests (SECURITY)** — sandbox-negative fixtures attempt prohibited path access, forbidden writes, symlink traversal including create-through-symlink ancestors, disallowed network egress including hostname/DNS attempts, forbidden environment inheritance/secret reads, interpreter misuse through allowed commands, out-of-phase tool use and protected-path access using the default list in `SECURITY.md`; expected-decision fixtures must validate against compiled policy artifacts in M0, and attempts must be denied by the compiled Linux OS policy once enforcement lands and by macOS policy-artifact parity checks until Seatbelt parity is implemented.
