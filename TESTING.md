...
- **M1.2 Executor and OS-isolation tests** — protocol conformance and both installation paths are platform-independent; the official Default Sandbox Executor runs the canonical hostile matrix on Ubuntu 24.04 x64 with stock Bubblewrap/seccomp. Productive Tool execution on macOS and Windows fails closed, and positive network grants are post-MVP. No test result certifies a Custom Executor.
