...
- **Event-ordering & transcript persistence (Loop Agent)** — tests cover the ADR-0059 `PROTOCOL.md` risks: append-before-publish and failure atomicity, pre-terminal observability, sequence/gap recovery, checkpoint durability, safe session IDs, and replay/tail/resume equivalence. Runtime logs are not project VCS/history.
